...
PCC_Lognormal_MeanUnknown <- function( data = NULL, historical_data = NULL,
                                       ...
                                       main = "PCC LogNormal with unknown mean")
{
  ...
  # HPRD - LogNormal likelihood ; Mean unknown
  HPRD_likLogNormMuU <- function(far, sdl, mp, sdp){

    f <- function(x){ qlnorm( 1-far+x, meanlog = mp, sdlog = sqrt( sdl^2 + sdp^2 ) ) -
                     qlnorm( x, meanlog = mp, sdlog = sqrt( sdl^2 + sdp^2 ) ) }
    out <- optimize( f, c(0, far), tol = .Machine$double.eps )
    c( qlnorm( out$minimum, meanlog = mp, sdlog = sqrt( sdl^2 + sdp^2 ) ),
       qlnorm( 1-far+out$minimum, meanlog = mp, sdlog = sqrt( sdl^2 + sdp^2 ) ) )
  }

  # Control limits
  if (!FIR) { CL <- t( mapply( function(MP, SDP, SDL, FD=FAR) { HPRD_likLogNormMuU(far = FD, sdl = SDL, mp = MP, sdp = SDP) },
                               SDL = sdl, MP = mu0_Post, SDP = sd_Post) )
  } else { CL <- t( mapply( function(MP, SDP, SDL, FD) { HPRD_likLogNormMuU(far = FD, sdl = SDL, mp = MP, sdp = SDP) },
                            SDL = sdl, MP = mu0_Post, SDP = sd_Post, FD = FAR) )
  }
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT <-
      ggplot2::ggplot(data.frame(x = c(
        ...
        ggplot2::aes(x = x)) +
      ggplot2::stat_function(fun = dnorm, args = list(mean = mu0_Post[N], sd = sd_Post[N]),
                             ggplot2::aes(colour = "Posterior", linetype = "Posterior"), size = 1) +
      {if(mu0 == 0 & sd0 == 10^6) { stat_function(fun = function(x) { dnorm(mu0_Post[N] + qnorm(.99)*sd_Post[N], mean = mu0_Post[N], sd = sd_Post[N]) }, ggplot2::aes(colour = "Prior", linetype = "Prior"), size = 1)
      } else { ggplot2::stat_function(fun = dnorm, args = list(mean = mu0, sd = sd0), ggplot2::aes(colour = "Prior", linetype = "Prior"), size = 1)  } }+
      ...
      ggplot2::ggtitle(expression(atop("PCC LogNormal likelihood - unknown mean"~theta, "Prior/Posterior distribution"))) +
      {if(mu0 == 0 & sd0 == 10^6) {
        ggplot2::geom_point( ggplot2::aes(x = mu0_Post[N], y = 0), color = "#3CB371",  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5 )
      } else { ggplot2::geom_point( ggplot2::aes(x = c(mu0, mu0_Post[N]), y = c(0, 0)), color = c("#FF4500", "#3CB371"),  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5 ) } } +
      {if(mu0 == 0 & sd0 == 10^6) {
...
#PCC_Lognormal_MeanUnknown(SimData, sdl = 1/2, PCC_PLOT = FALSE, PriorPosterior_PLOT = TRUE)
