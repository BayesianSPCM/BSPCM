# Function for running PCC Weibull with scale parameter unknown

PCC_Weibull <- function( data = NULL, historical_data = NULL,
                         kl = NULL, a0 = 0, b0 = 0, alpha_0 = NULL,
                         ...
                         main="PCC Weibull with unknown scale")
{
  ...
  # Likelihood kl input (i) more than one value for parameters (ii) non-numeric input
  if ( is.null(kl) ) {
    stop("Likelihood shape parameter 'kl' has not been defined")
  } else {
    if ( length(unlist(kl))>1 ) { message("More than one value for 'kl', the first one will only be used")
      if ( !is.numeric(kl) | kl<=0 ) { stop("Invalid 'kl' value") } else { kl <- kl[1] }
    } else { if ( !is.numeric(kl) | kl<=0 ) { stop("Invalid 'kl' value") } }
  }
  ...
  if ( !is.null(historical_data) ){
    ...
    a0_PowerP <- b0 + alpha_0*N_historicaldata
    b0_PowerP  <- b0 + alpha_0*sum(historical_data^k)
  ...
  ### PCC implementation
  # Sum of observations
  dataSumK <- cumsum( data^kl )[seq(1,length(data))]
  # Posterior distribution parameters
  a0_Post <- a0 + (1:N)
  b0_Post <- b0 + dataSumK


  # HPRD - Weibull likelihood ; scale parameter unknown
  HPRD_likWeibScU <- function(far, K, Ap, Bp){

    f <- function(x){
      actuar::qburr( 1-far+x, shape1 = Ap, shape2 = K, scale = Bp^(1/K) ) -
      actuar::qburr( x, shape1 = Ap, shape2 = K, scale = Bp^(1/K) )
      }
    out <- optimize( f, c(0, far), tol = .Machine$double.eps )
    c( actuar::qburr( out$minimum, shape1 = Ap, shape2 = K, scale = Bp^(1/K) ),
       actuar::qburr( 1-far+out$minimum, shape1 = Ap, shape2 = K, scale = Bp^(1/K)) )

  }


  # Control limits
  if (!FIR) { CL <- t( mapply( function(KL, A0, B0, FD=FAR) { HPRD_likWeibScU(far=FD, K=KL, Ap=A0, Bp=B0) }
                                  , KL=kl, A0=a0_Post, B0=b0_Post) )
  } else { CL <- t( mapply( function(K, A0, B0, FD) { HPRD_likWeibScU(far=FD, K=KL, Ap=A0, Bp=B0) }
                               , KL=kl, A0=a0_Post, B0=b0_Post, FD=FAR) )
  }
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT <-
      ggplot2::ggplot(data.frame(x = c(
        ...
        ifelse( a0 == 0 & b0 == 0, invgamma::qinvgamma(.9999, shape = a0_Post[N], rate = b0_Post[N]) + ifelse( a0_Post[N] > 1, b0_Post[N]/(a0_Post[N] - 1), b0_Post[N]/a0_Post[N] ),
                max( invgamma::qinvgamma(.9999, shape = a0, rate = b0), invgamma::qinvgamma(.9999, shape = a0_Post[N], rate = b0_Post[N]) ) + ifelse( a0 > 1, b0/(a0 - 1), b0/a0 ) )
      )
      ),
      ggplot2::aes(x = x)) +
      ...
      {if(a0 == 0 & b0 == 0) { ggplot2::stat_function(fun = function(x) { 1/(x + abs(log(ifelse( a0_Post[N] > 1, b0_Post[N]/(a0_Post[N] - 1), b0_Post[N]/a0_Post[N] ))))^kl }, ggplot2::aes(colour = "Prior", linetype = "Prior"), size = 1)
      } else { ggplot2::stat_function(fun = invgamma::dinvgamma, args = list(shape = a0, rate = b0), aes(colour = "Prior", linetype = "Prior"), size = 1) } } +
      ...
      ggplot2::scale_colour_manual(values = c("#3CB371", "#FF4500"),
                                   labels = c( bquote("Prior: IG(" ~ theta^k ~ "|" ~ .(round(a0, digits = 1)) ~ ", " ~ .(round(b0, digits = 1)) ~ ")" ),
                                               bquote("Posterior: IG(" ~ theta^k ~ "|" ~ .(round(a0_Post[N], digits = 1)) ~ ", " ~ .(round(b0_Post[N], digits = 1)) ~ ")" )),
                                   guide = guide_legend(override.aes = list( color = c("#FF4500", "#3CB371"),
                                                                             linetype = c("dashed", "solid"),
                                                                             size = c(.5, .5)), title = NULL)) +
      ggplot2::ggtitle(expression(atop("PCC Weibull likelihood - unknown scale"~theta^k, "Prior/Posterior distribution"))) +
      {if(a0 == 0 & b0 == 0) {
      ...
      {if(a0 == 0 & b0 == 0) {
        ggplot2::annotate( "text", x = ifelse( a0_Post[N] > 1, b0_Post[N]/(a0_Post[N] - 1), NA ), y = 0, label = paste(expression(mu[post])),
                          color = "#3CB371", size = 6, parse = TRUE, vjust = 1.25, na.rm = TRUE )
      } else { ggplot2::annotate( "text", x = c(ifelse( a0 > 1, b0/(a0 - 1), NA), ifelse( a0_Post[N] > 1, b0_Post[N]/(a0_Post[N] - 1), NA)), y = c(0, 0),
                      label = paste(expression(mu[prior], mu[post])), color = c("#FF4500", "#3CB371"), size = 6, parse = TRUE,
...
#out <- rweibull(n = 30, shape = 6.5, scale = 3)
#out[15] <- out[15] + 1.5
#PCC_Weibull(out, kl = 4.5, PCC_PLOT = FALSE, PriorPosterior_PLOT = TRUE)
