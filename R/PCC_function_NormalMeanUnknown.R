...
PCC_Normal_MeanUnknown <- function( data = NULL, historical_data = NULL,
                                    ...
                                    main = "PCC Normal with unknown mean")
{
  ...
  # HPRD - Normal likelihood ; Mean unknown
  HPRD_likNormMuU <- function(far, sdl, mp, sdp){
    c( qnorm( far/2, mp, sqrt(sdl^2 + sdp^2) ), qnorm( 1-far/2, mp, sqrt(sdl^2 + sdp^2) ) )
  }

  # Control limits
  if (!FIR) { CL <- t( mapply( function(MP, SDP, SDL, FD=FAR) { HPRD_likNormMuU(far = FD, sdl = SDL, mp = MP, sdp = SDP) },
                                  SDL = sdl, MP = mu0_Post, SDP = sd_Post) )
  } else { CL <- t( mapply( function(MP, SDP, SDL, FD) { HPRD_likNormMuU(far = FD, sdl = SDL, mp = MP, sdp = SDP) },
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT <-
      ...
      ggplot2::ggtitle(expression(atop("PCC Normal likelihood - unknown mean"~theta, "Prior/Posterior distribution"))) +
      {if(mu0 == 0 & sd0 == 10^6) {
...
#PCC_Normal_MeanUnknown(data = aPTT$aPTT_current, sdl = 0.5, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
