# Function for running PCC Pareto with unknown scale

PCC_Pareto <- function( data = NULL, historical_data = NULL,
                        ml = NULL, c0 = 0, d0 =0, alpha_0 = NULL,
                        ...
                        main="PCC Pareto with unknown scale")
{
  ...
  if ( is.null(ml) ) {
    stop("Likelihood shape (minimum) parameter 'ml' has not been defined")
  } else {
    if ( length(unlist(ml))>1 ) { message("More than one value for 'ml', the first one will only be used")
      if ( !is.numeric(ml) | ml<=0 ) { stop("Invalid 'ml' value") } else { ml <- ml[1] }
    } else { if ( !is.numeric(ml) | ml<=0 ) { stop("Invalid 'ml' value") } }
  }
  ...
  if ( !is.null(historical_data) ){
    ...
    c0_PowerP <- c0 + alpha_0*N_historicaldata
    d0_PowerP <- d0 + alpha_0*sum( log(historical_data/ml) )
  ...
  ### PCC implementation
  # Sum of observations
  datalogSum <- cumsum( log(data/ml) )[seq( 1, length(data) )]
  # Predictive distribution parameters
  c0_Post <- c0 + 1:N
  d0_Post <- d0 + datalogSum



  # HPRD - Pareto likelihood - scale parameters unknown
  HPRD_likParetoScU <- function(far, Ml, Cp, Dp){

    f <- function(x){ Ml*exp( extraDistr::qlomax(1-far+x, lambda = 1/Dp, kappa = Cp) ) -
                      Ml*exp( extraDistr::qlomax(x, lambda = 1/Dp, kappa = Cp) ) }
    out <- optimize( f, c(0, far), tol = .Machine$double.eps )
    c( Ml*exp( extraDistr::qlomax(out$minimum, lambda = 1/Dp, kappa = Cp) ),
       Ml*exp( extraDistr::qlomax(1-far+out$minimum, lambda = 1/Dp, kappa = Cp) ) )

  }

  # Control limits
  if (!FIR) { CL <- t( mapply( function(ML, CP, DP, FD=FAR) { HPRD_likParetoScU(far = FD, Ml = ML, Cp = CP, Dp = DP) }
                               , ML = ml, CP = c0_Post, DP = d0_Post) )
  } else { CL <- t( mapply( function(ML, CP, DP, FD) { HPRD_likParetoScU(far = FD, Ml = ML, Cp = CP, Dp = DP) }
                            , ML = ml, CP = c0_Post, DP = d0_Post, FD = FAR) )
  }
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT <-
      ...
      ggplot2::aes(x = x)) +
      ggplot2::stat_function(fun = dgamma, args = list(shape = c0_Post[N], rate = d0_Post[N]),
                             aes(colour = "Posterior", linetype = "Posterior"), size = 1) +
      {if(c0 == 0 & d0 == 0) { ggplot2::stat_function(fun = function(x) { 1/(x + abs(log(c0_Post[N]/d0_Post[N]))) }, ggplot2::aes(colour = "Prior", linetype = "Prior"), size = 1)
      } else { ggplot2::stat_function(fun = dgamma, args = list(shape = c0, rate = d0), ggplot2::aes(colour = "Prior", linetype = "Prior"), size = 1) } } +
      ...
      ggplot2::ggtitle(expression(atop("PCC Pareto likelihood - unknown scale"~theta, "Prior/Posterior distribution"))) +
      {if(c0 == 0 & d0 == 0) {
        ggplot2::geom_point( ggplot2::aes(x = c0_Post[N]/d0_Post[N], y = 0), color = "#3CB371",  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5, na.rm = TRUE )
      } else { ggplot2::geom_point( ggplot2::aes(x = c(c0/d0, c0_Post[N]/d0_Post[N]), y = c(0, 0)), color = c("#FF4500", "#3CB371"),  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5, na.rm = TRUE ) } } +
      {if(c0 == 0 & d0 == 0) {
        ggplot2::annotate("text", x = c0_Post[N]/d0_Post[N], y = 0, label = paste(expression(mu[post])), color = "#3CB371", size = 6, parse = TRUE, vjust = 1.25)
      } else { ggplot2::annotate("text", x = c(c0/d0, c0_Post[N]/d0_Post[N]), y = c(0, 0), label = paste(expression(mu[prior], mu[post])),
...
#SimData <- extraDistr::rpareto(n = 30, a = 4, b = 4)
#SimData[15] <- SimData[15]+1.5
#PCC_Pareto(SimData, ml=4, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE, pdf_report = FALSE)
