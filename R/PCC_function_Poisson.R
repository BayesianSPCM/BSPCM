                         ...
                         main = "PCC Poisson with rate unknown")
{
  ...
  # Control limits
  if (!FIR) { CL <- t( mapply( function(Rind, Pind, FARind=FAR) {
                                    do.call(HPRD_likPoissonRtU, list(far=FARind, R=Rind, P=Pind))
                                  }, Rind=R_Pred, Pind=P_Pred) )
  } else { CL <- t( mapply( function(Rind, Pind, FARind) {
                                do.call(HPRD_likPoissonRtU, list(far=FARind, R=Rind, P=Pind))
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT <-
      ...
      ggplot2::aes(x = x)) +
      ggplot2::stat_function(fun = dgamma, args = list(shape = c0_Post[N-1], rate = d0_Post[N-1]),
                             aes(colour = "Posterior", linetype = "Posterior"), size = 1) +
      {if(c0 == 1/2 & d0 == 0) { ggplot2::stat_function(fun = function(x) { 1/( sqrt(x) + abs(log(c0_Post[N-1]/d0_Post[N-1])) ) }, ggplot2::aes(colour = "Prior", linetype = "Prior"), size = 1)
      ...
      ggplot2::ggtitle(expression(atop("PCC Poisson likelihood - unknown rate"~theta, "Prior/Posterior distribution"))) +
      {if(c0 == 1/2 & d0 == 0) {
        ggplot2::geom_point( ggplot2::aes(x = c0_Post[N-1]/d0_Post[N-1], y = 0), color = "#3CB371",  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5, na.rm = TRUE )
      } else { ggplot2::geom_point( ggplot2::aes(x = c(c0/d0, c0_Post[N-1]/d0_Post[N-1]), y = c(0, 0)), color = c("#FF4500", "#3CB371"),  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5, na.rm = TRUE ) } } +
      {if(c0 == 1/2 & d0 == 0) {
        ggplot2::annotate("text", x = c0_Post[N-1]/d0_Post[N-1], y = 0, label = paste(expression(mu[post])), color = "#3CB371", size = 6, parse = TRUE, vjust = 1.25)
      } else { ggplot2::annotate("text", x = c(c0/d0, c0_Post[N-1]/d0_Post[N-1]), y = c(0, 0), label = paste(expression(mu[prior], mu[post])),
