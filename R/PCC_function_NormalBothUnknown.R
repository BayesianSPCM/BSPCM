                                    ...
                                    main = "PCC Normal with unknown parameters")
{
  ...
  # HPRD - Normal likelihood - both parameters unknown
  HPRD_likNormBU <- function(far, Mp, Lp, Ap, Bp){

      c( Mp + stats::qt( far/2, df=2*ifelse(Ap > 0, Ap, NA) ) * sqrt( ((Bp*(Lp+1)) / (Ap*Lp)) ),
         Mp + stats::qt( 1-far/2, df=2*ifelse(Ap > 0, Ap, NA) ) * sqrt( ((Bp*(Lp+1)) / (Ap*Lp)) ) )

  }

  # Control limits
  if (!FIR) { CL <- t( mapply( function(MU0, L0, A0, B0, FD=FAR) { HPRD_likNormBU(far = FD, Mp = MU0, Lp = L0, Ap = A0, Bp = B0) }
                                  , MU0 = mu0_Post, L0 = l0_Post, A0 = a0_Post, B0 = b0_Post) )
  } else { CL <- t( mapply( function(MU0, L0, A0, B0, FD) { HPRD_likNormBU(far = FD, Mp = MU0, Lp = L0, Ap = A0, Bp = B0) }
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT_mean <-
      ggplot2::ggplot(data.frame(x = c(
       ...
       ggplot2::aes(x = x)) +
      ggplot2::stat_function(fun = dnorm, args = list(mean = mu0_Post[N], sd = sqrt(1/l0_Post[N])),
                             ggplot2::aes(colour = "Posterior", linetype = "Posterior"), size = 1) +
      {if(mu0 == 0 & l0 == 0) { ggplot2::stat_function(fun = function(x) { dnorm(mu0_Post[N] + qnorm(.99)*sqrt(1/l0_Post[N]), mean = mu0_Post[N], sd = sqrt(1/l0_Post[N])) }, ggplot2::aes(colour = "Prior", linetype = "Prior"), size = 1)
        } else { ggplot2::stat_function(fun = dnorm, args = list(mean = mu0, sd = sqrt(1/l0)), aes(colour = "Prior", linetype = "Prior"), size = 1)  } }+
      ...
      ggplot2::ggtitle(expression(atop("PCC Normal likelihood - mean"~ theta[1] ~ "and variance"~ theta[2]^2 ~ " unknown", "Marginal distributions for"~ theta[1]))) +
      {if(mu0 == 0 & l0 == 0) {
        ggplot2::geom_point( aes(x = mu0_Post[N], y=0), color = "#3CB371",  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5 )
        } else { ggplot2::geom_point( aes(x = c(mu0, mu0_Post[N]), y=c(0, 0)), color = c("#FF4500", "#3CB371"),  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5 ) } } +
      {if(mu0 == 0 & l0 == 0) {
        ggplot2::annotate("text", x = mu0_Post[N], y = 0, label = paste(expression(mu[post])), color = "#3CB371", size = 6, parse = TRUE,
                          vjust = 1.25)
        } else {
          ggplot2::annotate("text", x = c(mu0, mu0_Post[N]), y = c(0, 0), label = paste(expression(mu[prior], mu[post])),
    ...
    PrPostPLOT_var <-
      ggplot2::ggplot(data.frame(x = c(
        ifelse( a0 == -1/2 & b0 == 0, invgamma::qinvgamma(1 - .9999, shape = a0_Post[N], rate = b0_Post[N]),
                min( invgamma::qinvgamma(1 - .9999, shape = a0, rate = b0), invgamma::qinvgamma(1 - .9999, shape = a0_Post[N], rate = b0_Post[N]) ) ),
        ifelse( a0 == -1/2 & b0 == 0, invgamma::qinvgamma(.9999, shape = a0_Post[N], rate = b0_Post[N]),
      ...
      {if(a0 == -1/2 & b0 == 0) { ggplot2::stat_function(fun = function(x) { 1/(x + .275)^2 }, ggplot2::aes(colour = "Prior", linetype = "Prior"), size = 1)
      ...
      ggplot2::scale_colour_manual(values = c("#3CB371", "#FF4500"),
                          ...
                          guide = guide_legend(override.aes = list( color = c("#FF4500", "#3CB371"),
                                                                    linetype = c("dashed", "solid"),
                                                                    size = c(.5, .5)), title = NULL)) +
      ggplot2::ggtitle(expression(atop("PCC Normal likelihood - mean"~ theta[1] ~ "and variance"~ theta[2]^2 ~ " unknown", "Marginal distributions for"~ theta[2]^2))) +
      {if(a0 == -1/2 & b0 == 0) {
    ...
    print(PrPostPLOT_mean) ; print(PrPostPLOT_var)
...
#PCC_Normal_BothUnknown(data = aPTT$aPTT_current, mu0 = 29.6, l0 = 1/7, a0 = 2, b0 = 0.56^2, PriorPosterior_PLOT = TRUE, pdf_report = TRUE)
#PCC_Normal_BothUnknown(data = aPTT$aPTT_current, mu0 = 35, l0=1/7, a0 = 1000, b0 = 2, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
#PCC_Normal_BothUnknown(data = aPTT$aPTT_current, mu0 = 35, l0=1/7, a0 = 2, b0 = 1000, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
#PCC_Normal_BothUnknown(data = aPTT$aPTT_current, mu0 = 35, l0=1/7, a0 = 100, b0 = 100, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
#PCC_Normal_BothUnknown(data = aPTT$aPTT_current, mu0 = 35, l0=1/7, a0 = 10, b0 = 2, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
#PCC_Normal_BothUnknown(data = aPTT$aPTT_current, mu0 = 35, l0=1/7, a0 = 2, b0 = 10, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
#PCC_Normal_BothUnknown(data = aPTT$aPTT_current, mu0 = 35, l0=2, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
