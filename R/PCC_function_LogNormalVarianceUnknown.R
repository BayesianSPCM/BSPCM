...
PCC_Lognormal_VarianceUnknown <- function( data = NULL, historical_data = NULL,
                                           ...
                                           main = "PCC LogNormal with unknown variance")
{
  ...
  # HPRD - LogNormal likelihood ; Variance unknown
  HPRD_likLogNormVarU = function(far, Ml, Ap, Bp){

    f <- function(x){ exp(Ml + qt( 1-far+x, df = 2*Ap )*sqrt(Bp/Ap)) -
                      exp(Ml + qt( x, df = 2*Ap )*sqrt(Bp/Ap) ) }
    out <- optimize(f, c(0, far), tol = .Machine$double.eps)
    c( exp(Ml + qt( out$minimum, df=2*Ap )*sqrt(Bp/Ap) ),
       exp(Ml + qt( 1-far+out$minimum, df=2*Ap )*sqrt(Bp/Ap) ) )

  }

  # Control limits
  if (!FIR) { CL <- t( mapply( function(MP, AP, BP, FD=FAR) { HPRD_likLogNormVarU(far = FD, Ml = MP, Ap = AP, Bp = BP) },
                               MP = ml, AP = a0_Post, BP = b0_Post) )
  } else { CL <- t( mapply( function(MP, AP, BP, FD) { HPRD_likLogNormVarU(far = FD, Ml = MP, Ap = AP, Bp = BP) },
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT <-
      ...
      ggplot2::ggtitle(expression(atop("PCC LogNormal likelihood - unknown variance"~theta^2, "Prior/Posterior distribution"))) +
      {if(a0 == 0 & b0 == 0) {
...
#PCC_Lognormal_VarianceUnknown(SimData, ml = 0, PCC_PLOT = FALSE, PriorPosterior_PLOT = TRUE)
