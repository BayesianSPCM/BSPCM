...
PCC_Lognormal_BothUnknown <- function( data = NULL, historical_data = NULL,
                                       ...
                                       main = "PCC LogNormal with unknown parameters")
{
  ...
  # HPRD - LogNormal likelihood - both parameters unknown
  HPRD_likLogNormBU <- function(far, Mp, Lp, Ap, Bp){

    f <- function(x){ if (Ap>0) {
                      exp( Mp + qt( 1-far+x, df = 2*ifelse(Ap > 0, Ap, NA) )*sqrt(  ( (Bp*(Lp+1))/(Ap*Lp) )  ) ) -
                      exp( Mp + qt( x, df = 2*ifelse(Ap > 0, Ap, NA) )*sqrt(  ( (Bp*(Lp+1))/(Ap*Lp) )  ) )
                      } else { 0 }
                    }
    out <- optimize(f, c(0, far), tol = .Machine$double.eps)
    c( exp( Mp + qt( out$minimum, df = 2*ifelse(Ap > 0, Ap, NA) )*sqrt( ( (Bp*(Lp+1))/(Ap*Lp)) ) ),
       exp( Mp + qt( 1-far+out$minimum, df = 2*ifelse(Ap > 0, Ap, NA) )*sqrt( ( (Bp*(Lp+1))/(Ap*Lp)) ) ) )

  }

  # Control limits
  if (!FIR) { CL <- t( mapply( function(MU0, L0, A0, B0, FD=FAR) { HPRD_likLogNormBU(far = FD, Mp = MU0, Lp = L0, Ap = A0, Bp = B0) }
                               , MU0 = mu0_Post, L0 = l0_Post, A0 = a0_Post, B0 = b0_Post) )
  } else { CL <- t( mapply( function(MU0, L0, A0, B0, FD) { HPRD_likLogNormBU(far = FD, Mp = MU0, Lp = L0, Ap = A0, Bp = B0) }
  ...
  if ( PriorPosterior_PLOT ) {
    PrPostPLOT_mean <-
      ...
      ggplot2::ggtitle(expression(atop("PCC LogNormal likelihood - mean"~ theta[1] ~ "and variance"~ theta[2]^2 ~ " unknown", "Marginal distributions for"~ theta[1]))) +
      {if(mu0 == 0 & l0 == 0) {
    ...
    PrPostPLOT_var <-
      ...
      ggplot2::scale_colour_manual(values = c("#3CB371", "#FF4500"),
                                   ...
                                   guide = guide_legend(override.aes = list( color = c("#FF4500", "#3CB371"),
                                                                             linetype = c("dashed", "solid"),
                                                                             size = c(.5, .5)), title = NULL)) +
      ggplot2::ggtitle(expression(atop("PCC LogNormal likelihood - mean"~ theta[1] ~ "and variance"~ theta[2]^2 ~ " unknown", "Marginal distributions for"~ theta[2]^2))) +
      {if(a0 == -1/2 & b0 == 0) {
...
#PCC_Lognormal_BothUnknown(SimData, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
