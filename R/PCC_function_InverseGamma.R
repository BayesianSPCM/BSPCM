...
PCC_InverseGamma <- function( data = NULL, historical_data = NULL,
                              ...
                              main="PCC Inverse-Gamma with unknown scale")
{
  ...
  # HPRD - InverseGamma likelihood - scale parameters unknown
  HPRD_likInvGScU = function(far, Al, Cp, Dp){

    f <- function(x){ 1/extraDistr::qbetapr( far-x, shape1 = Al, shape2 = Cp, scale = Dp ) -
                      1/extraDistr::qbetapr( 1-x, shape1 = Al, shape2 = Cp, scale = Dp ) }
    out <- optimize( f, c(0, far), tol = .Machine$double.eps )
    c( 1/extraDistr::qbetapr( 1-out$minimum, shape1 = Al, shape2 = Cp, scale = Dp ),
       1/extraDistr::qbetapr( far-out$minimum, shape1 = Al, shape2 = Cp, scale = Dp ) )

  }

  # Control limits
  if (!FIR) { CL <- t( mapply( function(AL, CP, DP, FD=FAR) { HPRD_likInvGScU(far = FD, Al = AL, Cp = CP, Dp = DP) }
                               , AL = al, CP = c0_Post, DP = d0_Post) )
  } else { CL <- t( mapply( function(AL, CP, DP, FD) { HPRD_likInvGScU(far = FD, Al = AL, Cp = CP, Dp = DP) }
                            , AL = al, CP = c0_Post, DP = d0_Post, FD = FAR) )
  }
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT <-
      ...
      ggplot2::ggtitle(expression(atop("PCC Inverse-Gamma likelihood - unknown rate"~theta, "Prior/Posterior distribution"))) +
      {if(c0 == 0 & d0 == 0) {
...
#PCC_InverseGamma(SimData, al = 3, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
