...
PCC_Gamma <- function(data = NULL, historical_data = NULL, al = NULL,
                      ...
                      main="PCC Gamma with rate unknown"){
  ...
  if ( !is.null(historical_data) ){
    ...
    c0_PowerP <- c0 + a*alpha_0*N_historicaldata
  ...
  # HPRD - Gamma likelihood ; rate unknown
  HPRD_likGamRtU <- function(far, A, Cp, Dp){

    f <- function(x) {extraDistr::qbetapr( 1-far+x, shape1 = A, shape2 = Cp, scale = Dp ) - extraDistr::qbetapr( x, shape1 = A, shape2 = Cp, scale = Dp )}
    out <- optimize( f, c(0,far), tol = .Machine$double.eps )
    c( extraDistr::qbetapr(out$minimum, shape1 = A, shape2 = Cp, scale = Dp), extraDistr::qbetapr(1-far+out$minimum, shape1 = A, shape2 = Cp, scale = Dp) )

  }

  # Control limits
  if (!FIR) { CL <- t( mapply( function(A0, C0, D0, FD=FAR) { HPRD_likGamRtU(far=FD, A=A0, Cp=C0, Dp=D0) }
                                  , A0=al, C0=c0_Post, D0=d0_Post) )
  } else { CL <- t( mapply( function(A0, C0, D0, FD) { HPRD_likGamRtU(far=FD, A=A0, Cp=C0, Dp=D0) }
  ...
  if ( PriorPosterior_PLOT ) {
    ...
    PrPostPLOT <-
      ...
      ggplot2::ggtitle(expression(atop("PCC Gamma likelihood - unknown rate"~theta, "Prior/Posterior distribution"))) +
      {if(c0 == 0 & d0 == 0) {
...
#out[15] <- out[15] + 1.5
#PCC_Gamma(out, al = 4, PriorPosterior_PLOT = TRUE, PCC_PLOT = FALSE)
